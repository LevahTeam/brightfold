...
seedUser("volunteer", "Volunteer", "QTP_SEED_VOLUNTEER_PW", "member");
seedUser("pastor", "Pastor", "QTP_SEED_PASTOR_PW", "admin");

if (demo) {
  console.log("\nDemo data");
  const grade = createGrade("5th Grade", 50);
  const cls = createClass(grade.id, "A 5-1 (Demo)", "Ms. Demo Teacher", 10);
  ...
  const weeks = weekLabels.map(([label, date]) => upsertWeek(grade.id, label, date));
  ...
  for (const [english, korean, pages] of kids) {
    const { id } = upsertKid(cls.id, english, korean);
    pages.forEach((p, i) => {
      setEntry(id, weeks[i].id, p === null ? "ABSENT" : "HERE", p ?? 0, "seed");
    });
  }
